...
public class DataValidation {
	...
	private static final String ARS_GENERATED_ID_PATTERN = "^([A-Z]{2})";
	public static final String ENROLMENTNUMBER_PATTERN = "^([A-Z]{2})/[0-9]{4}/([0-9][0-9])?[0-9][0-9]$";
	...
	public boolean isARSGeneratedIdValid(final String ARSGeneratedId) {
		Pattern pattern = Pattern.compile(ARS_GENERATED_ID_PATTERN);
		matcher = pattern.matcher(ARSGeneratedId);
		return matcher.matches();

	}
	public boolean isRegisteredIdValid(final String regid) {
		Pattern pattern = Pattern.compile(ENROLMENTNUMBER_PATTERN);
		matcher = pattern.matcher(regid);
		return matcher.matches();
